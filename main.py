...
def runPFEncryption(key,msg,output):
    ...
    while i<len(msg):
        ...
        if loc[1]==loc1[1]:
            ans = ans + encryptedMatrix[(loc[0]+1)%5][loc[1]] + encryptedMatrix[(loc1[0]+1)%5][loc1[1]]
#             print("{}{}".format(encryptedMatrix[(loc[0]+1)%5][loc[1]],encryptedMatrix[(loc1[0]+1)%5][loc1[1]]).lower(),end='')
        elif loc[0]==loc1[0]:
            ans = ans + encryptedMatrix[loc[0]][(loc[1]+1)%5] + encryptedMatrix[loc1[0]][(loc1[1]+1)%5]
...
def decryptPLayfair():
    ...
    b1 = Button(decryptPlayfairWindow,text='Submit',width=12,pady=5,command =lambda: runPFDecryption(key_text.get(),message_text.get(),output))
    b1.grid(row=5,column=5)
